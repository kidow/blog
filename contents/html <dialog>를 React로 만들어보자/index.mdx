...
date: 2022-06-18
description: 기존에는 dialog나 modal을 하나부터 열까지 직접 구현했는데, 이제는 dialog 태그로 더욱 좋은 접근성으로 만들 수 있게 되었습니다.
thumbnail: https://opengraph.kidow.me/api?id=hijjy21h309
keywords: React
---

`<dialog>` 태그에 대해 아시나요? 기본적으로 html tag로서 존재하는 이 태그는 우리가 흔히 아는 modal과 같은 기능을 담당합니다. 이 좋은 걸 사람들이 잘 쓰지 않았던 이유는, 바로 같잖은 **Internet Explorer** 때문이었죠.

![IE의 마지막. 인터넷 커뮤니티 '클리앙' 캡쳐](ie.jpeg)
...
dialog 태그 안에서 form[method='dialog']를 넣으면, 제출 시에 `returnValue`라는 값을 추출하는 것이 가능합니다. 이 경우는 리액트에서는 막 필요한 기능은 아닙니다. 데이터 통신은 props로 해도 되니까요.
...
해당 컴포넌트는 [이 곳](https://components.kidow.me)에서 마음껏 테스트하실 수 있습니다. 🤗
